...
import { getWithdrawalHistory, cancelWithdrawal } from '../../services/api';
import type { WithdrawalHistoryItem } from '../../types';
import { motion } from 'framer-motion';
import { toast } from 'react-toastify';
import { ArrowUpCircle, CheckCircle2, XCircle, Clock, Loader, History, X } from 'lucide-react';
...
const statusConfig = {
  ...
  canceled: { text: 'Cancelado', icon: <XCircle size={16} />, className: 'text-red-400 bg-red-500/10' },
};
  ...
  return (
    <div className="space-y-3">
      {withdrawals.map((item, index) => {
        ...
        const isCancellable = item.status === 'pending' || item.status === 'processing';
            ...
            <div className="flex items-center gap-4">
              ...
              {isCancellable && (
                <button
                  ...
                  className="flex items-center gap-1 text-sm font-semibold px-3 py-1 rounded-full text-red-400 bg-red-500/10 hover:bg-red-500/20 transition-colors"
                >
                  <X size={16} />
                  <span>Cancelar</span>
                </button>
