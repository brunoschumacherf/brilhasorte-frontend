...
const Profile: React.FC = () => {
  const { user, updateUserDetails } = useAuth(); // Obtém a função para atualizar o usuário
  ...
  // Esta função será chamada pelo EditProfileForm APÓS o sucesso da API
  const handleSave = (updatedUser: Partial<User>) => {
    updateUserDetails(updatedUser); // Atualiza o estado global do usuário
      ...
      <div className="bg-[var(--surface-dark)] border border-[var(--border-color)] shadow-lg overflow-hidden sm:rounded-lg">
        {isEditing ? (
          ...
          <>
            ...
            <div className="border-t border-[var(--border-color)] px-4 py-5 sm:p-0">
              <dl>
                ...
                <DetailItem label="Data de Nascimento" value={user.birth_date ? new Date(user.birth_date).toLocaleDateString('pt-BR', { timeZone: 'UTC' }) : 'Não informado'} />
