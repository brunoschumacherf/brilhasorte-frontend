...
const WithdrawalList: React.FC = () => {
  ...
  const [isApproving, setIsApproving] = useState<string | null>(null); // Tracks which withdrawal is being approved
  ...
  const handleApprove = async (withdrawalId: string) => {
    ...
    try {
      ...
      setWithdrawals(prev =>
        prev.map(w =>
          w.id === withdrawalId ? { ...w, attributes: { ...w.attributes, status: 'completed' } } : w
        )
      );
    } catch (error) {
          ...
          <table className="min-w-full divide-y divide-gray-700">
            ...
            <tbody className="bg-gray-800 divide-y divide-gray-700">
              {loading ? <TableSkeleton cols={6} /> : withdrawals.map((item) => {
                ...
                const isPending = attributes.status === 'pending';
                return (
                  <tr key={item.id} className="hover:bg-gray-700">
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{findUserEmail(relationships.user)}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-red-400">{formatCurrency(attributes.amount_in_cents)}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                      <div>{attributes.pix_key}</div>
                      <div className="text-xs uppercase font-medium text-gray-500">{attributes.pix_key_type}</div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-400">{new Date(attributes.created_at).toLocaleString('pt-BR')}</td>
