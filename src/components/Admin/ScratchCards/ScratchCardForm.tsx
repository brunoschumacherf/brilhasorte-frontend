...
const prizeSchema = z.object({
  ...
  value_in_cents: z.number().min(0, "O valor deve ser positivo."),
  probability: z.number().min(0, "A probabilidade deve ser positiva.").max(1, "A probabilidade não pode ser maior que 1."),
  stock: z.number().int("O stock deve ser um número inteiro."),
...
const scratchCardSchema = z.object({
  ...
  price_in_cents: z.number().min(0, "O preço deve ser positivo."),
...
const ScratchCardForm: React.FC<ScratchCardFormProps> = ({ isOpen, onClose, onSave, existingScratchCard }) => {
  ...
  const totalProbability = (prizesWatcher || []).filter(p => !p._destroy)
  ...
  const onSubmit: SubmitHandler<ScratchCardFormData> = async (data) => {
    const payload = {
      ...data,
      price_in_cents: Math.round(toNumber(data.price_in_cents) * 100),
      prizes_attributes: data.prizes.map(p => ({
        ...p,
        value_in_cents: Math.round(toNumber(p.value_in_cents) * 100),
        probability: toNumber(p.probability),
        stock: Math.floor(toNumber(p.stock))
      })),
    };
    // @ts-ignore
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 max-h-[80vh] overflow-y-auto p-1 pr-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input {...register("price_in_cents", { setValueAs: toNumber })} type="number" step="0.01" className={inputClasses(!!errors.price_in_cents)} />
        ...
        <div className="border-t border-white/10 pt-4 mt-4">
              ...
              <div key={field.id} className="p-4 border border-white/10 rounded-lg bg-white/5 relative">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input {...register(`prizes.${index}.value_in_cents`, { setValueAs: toNumber })} type="number" step="0.01" className={inputClasses(!!errors.prizes?.[index]?.value_in_cents)} />
                  </div>
                  <div>
                    <label className="text-xs font-medium text-gray-400">Probabilidade (0.0 a 1.0)</label>
                    <input {...register(`prizes.${index}.probability`, { setValueAs: toNumber })} type="number" step="0.001" className={inputClasses(!!errors.prizes?.[index]?.probability)} />
                  </div>
                  <div>
                    <label className="text-xs font-medium text-gray-400">Stock (-1 para infinito)</label>
                    <input {...register(`prizes.${index}.stock`, { setValueAs: toNumber })} type="number" className={inputClasses(!!errors.prizes?.[index]?.stock)} />
                  </div>
          ...
          <button
            type="button"
            onClick={() => append({ name: '', value_in_cents: 0, probability: 0, stock: -1, image_url: '' })}
            className="mt-4 flex items-center gap-2 text-sm font-semibold text-yellow-400 hover:text-yellow-300"
          >
